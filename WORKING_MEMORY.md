...
- Homepage daily briefing is only "live" when a company `ai_summaries` row exists for today's UTC `summary_date`; older rows must not be shown as the current daily briefing.
